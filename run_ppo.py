...
def train_model(timesteps):
    ...
    env_train = StockEnvTrade(data, model_name="PPO", train=True)
    
    model = PPO("MlpPolicy", env_train)
    model.learn(timesteps)
    model.save("ppo_{}".format(timesteps))
...
def test_model(timesteps):
    ...
    env_test = StockEnvTrade(data, model_name="PPO", train=False)
    
    model = PPO.load("ppo_{}".format(timesteps))
